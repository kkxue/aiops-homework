...
// 删除 K8s 资源，课后作业
func deleteResource(namespace, resourceType string, resourceName string) (string, error) {
	fmt.Printf("call delete resource: %s %s %s\n", namespace, resourceType, resourceName)
	clientGo, err := utils.NewClientGo(kubeconfig)
	resourceType = strings.ToLower(resourceType)
	var gvr schema.GroupVersionResource
	switch resourceType {
	...
	// delete the resources using the dynamic client
